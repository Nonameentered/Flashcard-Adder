...
class FieldStackView: UIStackView {
    ...
    init(fieldName: String, existingName: String? = nil, axis: NSLayoutConstraint.Axis = .horizontal) {
        super.init(frame: .zero)
        setUpView(fieldName: fieldName, axis: axis)
        if existingName != nil {
            textView.text = existingName
        }
    }
...
class NoteView: UIStackView {
    ...
    private func setUpView(initialNote: Note?, axis: NSLayoutConstraint.Axis) {
        ...
        if let initialNote = initialNote {
            titleView = FieldStackView(fieldName: "Note Name", existingName: initialNote.name)
            isCloze = initialNote.acceptsCloze
            fieldViews = initialNote.fields.enumerated().map { (index, element) -> FieldStackView in
                return FieldStackView(fieldName: "Field \(index + 1) Name", existingName: element.name)
            }
